...
function AdminPage() {
  ...
  const runSetProjectCover = useServerFn(setProjectCover);
  ...
  const coverMutation = useMutation({
    mutationFn: (input: { projectId: string; photoId: string | null }) => runSetProjectCover({ data: input }),
    onSuccess: async () => {
      setMessage("Cover image updated.");
      await queryClient.invalidateQueries({ queryKey: ["admin-projects"] });
    },
    onError: (error) => setMessage(error instanceof Error ? error.message : "Cover could not be updated."),
  });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-5xl px-6 py-12">
        ...
        {activeProjectId && <section className="mt-16"><div className="mb-6 flex items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Selected project</p><h2 className="mt-2 font-heading text-2xl font-medium tracking-tight">{projects.find((project) => project.id === activeProjectId)?.title}</h2></div><select aria-label="Add photo to project" disabled={availablePhotos.length === 0 || assignmentMutation.isPending} value="" onChange={(event) => { if (event.target.value) assignmentMutation.mutate({ projectId: activeProjectId, photoId: event.target.value, sortOrder: assignedPhotos.length }); }} className="max-w-[220px] border-b border-border bg-background py-2 text-sm outline-none"><option value="">Add photograph…</option>{availablePhotos.map((photo) => <option key={photo.id} value={photo.id}>{photo.alt || photo.id.slice(0, 8)}</option>)}</select></div><div className="mb-8 flex flex-wrap items-center gap-4 border-b border-border pb-6"><span className="text-sm text-muted-foreground">Cover image</span>{(() => { const active = projects.find((project) => project.id === activeProjectId); const coverId = active?.coverPhotoId ?? assignedPhotos[0]?.id ?? null; const cover = assignedPhotos.find((photo) => photo.id === coverId); return cover ? <img src={cover.src} alt={cover.alt || "Project cover"} className="h-16 w-24 flex-none object-cover" loading="lazy" /> : <span className="text-sm text-muted-foreground">None</span>; })()}<select aria-label="Project cover image" disabled={assignedPhotos.length === 0 || coverMutation.isPending} value={projects.find((project) => project.id === activeProjectId)?.coverPhotoId ?? ""} onChange={(event) => coverMutation.mutate({ projectId: activeProjectId, photoId: event.target.value || null })} className="max-w-[220px] border-b border-border bg-background py-2 text-sm outline-none"><option value="">First photograph (automatic)</option>{assignedPhotos.map((photo) => <option key={photo.id} value={photo.id}>{photo.alt || photo.id.slice(0, 8)}</option>)}</select></div><div onDragOver={(event) => { event.preventDefault(); setProjectDragging(true); }} onDragLeave={() => setProjectDragging(false)} onDrop={(event) => { event.preventDefault(); setProjectDragging(false); void uploadFiles(event.dataTransfer.files, activeProjectId); }} onClick={() => projectFileInput.current?.click()} className={`mb-8 flex cursor-pointer flex-col items-center justify-center rounded-sm border border-dashed px-4 py-8 text-center transition-colors sm:px-6 sm:py-10 ${projectDragging ? "border-foreground bg-muted/40" : "border-border hover:border-foreground/40"}`}><p className="font-heading text-base font-medium">{uploading ? "Optimising & uploading…" : "Upload photos to this project"}</p><p className="mt-2 text-sm text-muted-foreground">Drop files or click to choose — added to this project automatically</p><input ref={projectFileInput} type="file" accept="image/*" multiple className="hidden" onChange={(event) => event.target.files && void uploadFiles(event.target.files, activeProjectId)} /></div><ul className="divide-y divide-border border-y border-border">{assignedPhotos.map((photo, index) => <li key={photo.projectPhotoId} className="flex flex-wrap items-center gap-x-4 gap-y-3 py-4"><img src={photo.src} alt={photo.alt || "Portfolio photograph"} className="h-16 w-24 flex-none object-cover" loading="lazy" /><span className="min-w-0 flex-1 text-sm">{photo.alt || "Untitled photograph"}</span><label className="flex flex-none items-center gap-2 text-sm text-muted-foreground"><input type="checkbox" checked={photo.showOnHomepage} disabled={homepageMutation.isPending} onChange={(event) => homepageMutation.mutate({ id: photo.projectPhotoId, showOnHomepage: event.target.checked })} /> Show on homepage</label><input type="number" min="0" value={photo.projectSortOrder} aria-label={`Project order for photograph ${index + 1}`} onChange={(event) => reorderMutation.mutate({ id: photo.projectPhotoId, sortOrder: Number(event.target.value) })} className="w-16 border-b border-border bg-transparent py-1 text-sm outline-none" /><button onClick={() => removeAssignmentMutation.mutate(photo.projectPhotoId)} className="text-sm text-muted-foreground underline underline-offset-4 hover:text-leica-red">Remove</button></li>)}</ul>{assignedPhotos.length === 0 && <p className="py-8 text-sm text-muted-foreground">Assign photographs to this project from the menu above.</p>}</section>}
