...
export const Route = createFileRoute("/work/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      ...
      links: [{ rel: "canonical", href: `${SITE_URL}/work/${loaderData?.slug ?? ""}` }],
    };
  },
...
function ProjectPage() {
  ...
  if (!project) return <ProjectMessage title="Project not found" detail="The project may have moved or is no longer published." />;

  return (
    <main className="min-h-screen bg-background px-5 pb-28 pt-32 text-foreground md:px-6 md:pt-40">
      <div className="mx-auto w-full max-w-[1800px]">
        <div className="mb-16 flex items-baseline justify-between gap-6 md:mb-24">
          <div>
            <Link to="/work" className="text-sm text-muted-foreground transition-colors hover:text-foreground">
              ← Work
            </Link>
